...
describe("setup companion-shim diagnosis", () => {
  ...
  it("cmdSetup appends the shim diagnosis (source guard)", () => {
    ...
    const cmdSetupSource = source.slice(source.indexOf("async function cmdSetup("), source.indexOf("function cmdStatus("));
...
describe("kusabi-companion extraction invariants (kusabi #437)", () => {
  it("companion contains no moved command definitions or exports", () => {
    ...
    assert.ok(!source.includes("async function cmdTask("), "cmdTask must not be defined in companion");
    assert.ok(!source.includes("async function cmdReview("), "cmdReview must not be defined in companion");
    assert.ok(!source.includes("buildTaskReviewInput("), "buildTaskReviewInput must not be in companion");
