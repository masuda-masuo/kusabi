...
9. **kaiba**: `kaiba_recall` lets you look up what earlier phases concluded, `kaiba_agenda` lists the shared queue (read-only), and `kaiba_progress` lets you record in-flight notes. `kaiba_remember` is not allowed — a durable fact you turn up goes in your final report for the orchestrator to file.
