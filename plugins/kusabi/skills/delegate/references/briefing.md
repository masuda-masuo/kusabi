...
- **Annotate `baseline-red` exactly when the line is expected to be red on the checkout as handed to the worker.** Not only on a file that does not exist: the annotation is read off the bullet alone, and the two common cases are a Deliverables file not yet existing (`node --check ui/app.js` on a greenfield task) and a test not yet existing inside a file that does (`pytest tests/test_codex.py -q -k failed`, exit 5 today). The machine verifies the claim rather than trusting it: a measured mismatch at base dispatches, an entry already green at base refuses (the annotation is wrongable in exactly one direction so it cannot rot silently), and an unmeasurable run refuses fail-closed. `exit <N>` and `baseline-red` compose. Reading this rule as file-creation-only costs the strongest smoke line available for "write a test covering the failure path" and demotes it into unverifiable prose under `## Acceptance criteria` (kusabi #516).
- **The annotation rescues only a red the round removes.** The pre-dispatch probe runs each command verbatim on the pristine checkout and waits for no setup the brief asks the worker to perform, so a line red because the container lacks a package is still red after the round and is refused — correctly. **Every Smoke line must be self-contained**: `python -m pip install -q pyyaml && python -c '...'`, not a bare import with "install pyyaml" written in the prose. Ignoring this costs a refused dispatch whose reason reaches only the detach log (kusabi #513).
