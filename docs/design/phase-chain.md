...
Pure function `deriveDisposition({verdict, probesGreen, round, maxRounds, repeatedAreas, findingSeverities, strategizeEligible, oracleViolation, refusal})` in `plugins/kusabi/scripts/disposition.mjs`:
...
This complements the authoring-side guards rather than replacing them: the delegate skill's freeze-time grep rule (#290) prevents one contradiction class before dispatch, and the brief lint (#289) and smoke baseline probe (#292) catch mechanical and runtime defects. What is left is the residue those cannot catch — contradictions only discoverable mid-implementation. Implement seat only for now; the review seat has no equivalent.
