...
```
agy -p <prompt> --output-format json --model <id> [--json-schema <schema>] [--conversation <id>]
```
...
conversation id is supplied; a fresh dispatch's argv is byte-identical to
v1's. The id recorded on the job is exactly what the CLI returns as
