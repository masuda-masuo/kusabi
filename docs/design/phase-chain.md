...
**Stats are structural, not measured (kusabi #215).** v1 has no event stream (`--output-format json`), so the claude job record's `stats` object is marked `instrumented: false` with null/empty counters (`events`, `steps`, `lastTool`, `permissionsAllowed`, `permissionsRejected`, `lastActivity` all `null`, `models: []`) — distinguishable from the opencode path's measured zeros by the marker alone, never by guessing from `backend`. Counters stay present (not absent) so existing readers keep working unchanged: `?? 0` / `?? "-"` rendering, the serve-lifecycle idle-reap fallback (`stats.lastActivity ?? startedAt`), and `/kusabi:status` (which renders "stats: not instrumented (no event stream in v1)" instead of structural zeros). Records without the marker (opencode and legacy) render the counters as before.
