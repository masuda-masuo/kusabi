...
The same dispatch-time brief lint (`briefLintReport`, under `chain || phase`) enforces premise discipline and evidence self-proof (kusabi #536). When `## Premises` is present, each bullet must carry a measurement with a count (`measured:`) and reference an existing section as a guard (`guard:`); empty headings, missing numbers, or ungrounded guards are refused before dispatch. Additionally, any evidence path matching `/tmp/...` in `## Workplace` must be read by at least one command in `## Smoke`, ensuring shipped fixtures prove themselves (preventing lost rounds from empty sample files).
...
Review parse/run (`parseReviewResult`, `runReviewPhase`, `shouldSkipReview`, `buildReviewRepairPrompt`, `renderProbeReport`, `renderReviewPriorFindings`) live in `plugins/kusabi/scripts/chain-review.mjs` (kusabi #435). Persist/record writing (`computeChainTotals`, `persistChainState`, `writeReviewRecord`) lives in `plugins/kusabi/scripts/chain-persist.mjs` (kusabi #451). Production callers are `chain-finish.mjs`, `chain-driver.mjs`, and `chain-outcomes.mjs`; `chain-phases.mjs` does not import `chain-persist.mjs`. Quota classification (`classifyDispatchQuotaExhaustion`, `quotaExhaustionReason`, `quotaReplacementRefusal`, `recordQuotaExhaustion`, `explicitRouteDiffersFromRecord`) lives in `plugins/kusabi/scripts/chain-quota.mjs` (kusabi #453). Production callers are `chain-run.mjs`, `chain-review.mjs`, `chain-resume-resolve.mjs`, `chain-cmd.mjs`, and `chain-finish.mjs`; `chain-phases.mjs` does not import `chain-quota.mjs`. The production caller of `runReviewPhase` is `chain-finish.mjs`; `chain-phases.mjs` does not import `chain-review.mjs`. Terminal outcome rendering (`renderAcceptOutcome`, `renderAcceptWithFollowupOutcome`, `renderEscalateOutcome`, `renderRefusalOutcome`, `renderBriefSyntaxDefectOutcome`, `renderMaxRoundsOutcome`, `renderProviderExhaustedOutcome`, `handleProviderExhaustion`) lives in `plugins/kusabi/scripts/chain-outcomes.mjs` (kusabi #439). Production callers are `chain-finish.mjs` and `chain-driver.mjs`; `chain-phases.mjs` does not import `chain-outcomes.mjs`. Resume position and replacement-seat archive (`resolveChainResume`, `classifyReviewSeatReplacement`, `archiveFailedReviewSeat`) live in `plugins/kusabi/scripts/chain-resume-resolve.mjs` (kusabi #441). Production callers are `chain-cmd.mjs` and `chain-driver.mjs`; `chain-phases.mjs` does not import `chain-resume-resolve.mjs`. Implement and probe orchestration (`withContainerWorkspace`, `buildImplementText`, `runImplementPhase`, `runProbePhase`) live in `plugins/kusabi/scripts/chain-run.mjs` (kusabi #447). Production callers are `chain-driver.mjs` and `task-cmd.mjs`; `chain-phases.mjs` does not import `chain-run.mjs`. Container-context collection (`collectChangeScope`, `collectContainerBaseContext`, `collectContainerReviewInput`, `collectReviewContext`, `assertContainerBaseRef`) lives in `plugins/kusabi/scripts/chain-collect.mjs` (kusabi #449). Production callers are `chain-run.mjs`, `chain-driver.mjs`, and `task-cmd.mjs`; `chain-phases.mjs` does not import `chain-collect.mjs`. Strategize dispatch (`runStrategizePhase`) lives in `plugins/kusabi/scripts/chain-strategize.mjs` (kusabi #455). The production caller is `chain-finish.mjs`; `chain-phases.mjs` does not import `chain-strategize.mjs`.
...
This complements the authoring-side guards rather than replacing them: the delegate skill's freeze-time grep rule (#290) prevents one contradiction class before dispatch, and the brief lint (#289) and smoke baseline probe (#292, #315) catch mechanical and runtime defects — the baseline refuses an unannotated smoke entry that is red on the pristine checkout, and refuses a `baseline-red` entry that is GREEN there (the annotation licenses exactly one baseline outcome, a measured mismatch: the entry is declared red on the pristine checkout, typically because a deliverable or a test it names does not exist yet; green means the brief is stale or the work already exists), while an annotated entry red at base as declared dispatches. What is left is the residue those cannot catch — contradictions only discoverable mid-implementation. Implement seat only for now; the review seat has no equivalent.
...
**Exit-code contract.** Exit 0 means the chain reached a terminal state — *whatever* its disposition. Reporting the disposition is the digest's job and judging it is the orchestrator's, so a chain that escalated exits 0 here. A non-zero exit means the **wait itself** failed, never a disposition you dislike; `ChainWaitError.code` is the machine-readable half: `no-chain-appeared`, `stalled`, `usage` (the former `unknown-chain` is gone — a named chain is never "unknown", it either appears or times out). A named wait on a chain whose directory does not exist yet polls for it within the appear window — the launcher mints the id in the parent and hands it back before the child has created the directory, so "not there yet" is the normal shape — and throws `no-chain-appeared` on expiry, naming the id and both live possibilities: the launcher may still be in pre-flight, or it exited without ever creating the chain (since kusabi #513 a refused dispatch exits non-zero in the parent, so that case means the spawned child died). A malformed id (not `chain-*`, or carrying a path separator) is refused immediately as `usage`, with no polling. A dispatch that died before creating a chain directory therefore exits non-zero instead of looking finished — failure mode 2 above, closed.
...
**Bounds.** Poll interval 2s (`--poll-interval`). `--appear-timeout` 120s bounds `--next`, a named chain that has not appeared yet (kusabi #514), and a chain directory that never gets a control record. `--progress-timeout` defaults to **2 hours** and is deliberately generous: it is the backstop for a chain whose process is alive but making no progress, while the fast path for the common death (process gone mid-round) is the liveness probe, which does not wait for it.

**`--next`** waits for a chain to APPEAR instead of naming one — the mode for the genuinely unnamed case: attaching to a chain someone else launched, or a dispatch that does not hand an id back (since kusabi #514 `chain-detach` mints the id in the parent and names it, so its own wait line never needs `--next`). It selects the newest chain that is either new since the wait started or was already there and unfinished, so a chain created in the moment before the wait started still counts while one that finished earlier never does. A pre-existing empty directory with no control record, older than `--appear-timeout`, is debris from a dispatch that died before writing anything and is skipped with a stderr note; while the selected chain is still recordless a newer or same-stamped chain that appears wins, so the wait cannot stall on an empty directory (a directory once traded away is never revisited). `--since <ISO>` is the precise tool when several chains run in one workspace at once and newest-unfinished selection would be ambiguous.

**`chain-detach` — background launch and wait handoff.** Launches a chain in a detached background process and prints a runnable `chain-wait` command line (`kusabi-companion chain-wait <chainId> ...`), eliminating hand-rolled backgrounding and PID tracking. The chain id is minted in the parent (kusabi #514) and passed to the child via `--chain-id`, so the emitted wait line names exactly the chain this launcher spawned — no `--next`, no `--since`, and no recency race with another orchestrator working the same repo (the state root is keyed by cwd, not container, so `--next` selection could latch onto a chain launched from the same repo by someone else). A caller-supplied `--chain-id` is caller-owned and must be unique per concurrent dispatch; the parent check is not a reservation, so concurrent duplicates are detected only by the child's atomic claim, whose refusal (logged) names the situation (kusabi #522). Pre-flight checks (brief-file resolution, publish warning, lossy-smoke refusal, `--chain-id` shape validation, session provenance, container requirement, and dispatch-time brief lint) run before child spawn so invalid dispatches exit non-zero immediately without spawning a background child or printing a wait command line. It accepts the same flags as `chain` (`--container <cid>`, `--model <identifier>`, `--backend opencode|claude|agy|cursor`, `--brief-file <path>`, `--max-rounds <N>`, `--session <id>`, `--timeout <s>`, `--watchdog <s>`, `--deny <tools>`, `--keep-serve`, `--chain-id <id>`) and forwards tracking flags (`--appear-timeout <s>`, `--poll-interval <s>`, `--progress-timeout <s>`) into the generated `chain-wait` invocation. The operator (or orchestrator harness) waits on the **chain id** via `chain-wait`, not on a hand-rolled process watcher.
...
`chain-detach` and `chain-wait` (§3.5.7b) provide background dispatch and tracking primitives for iterative multi-round chains. Single-phase standalone tasks (`task`) previously lacked symmetric background and tracking mechanisms, requiring orchestrator harnesses or external dispatchers to hand-roll process management and state polling.

**`task-detach` — background single-task launch.** Launches a single `task` in a detached background process, redirecting stdout and stderr to a log file (`task-detach-<timestamp>.log`) in the workspace state directory, and prints the exact runnable `task-wait` command line (`kusabi-companion task-wait --next --since <ISO> ...`). Like `chain-detach`, `task-detach` performs all synchronous pre-flight checks (brief file resolution, lossy-smoke refusal, phase/agent validation, session resolution, tool restrictions, and brief lint) before spawning the child process. Invalid invocations exit non-zero immediately without spawning a detached child or printing a wait command line. All standard task dispatch flags (`--container`, `--brief-file`, `--phase`, `--agent`, `--model`, `--backend`, `--timeout`, `--watchdog`, `--read-only`, `--deny`, `--session`, `--resume-last`) are forwarded to the child `task` invocation, while tracking options (`--next`, `--since`, `--appear-timeout`, `--poll-interval`, `--progress-timeout`) are mapped to the printed `task-wait` command.

**`task-wait` — task completion detection.** A trackable, lightweight state watcher that monitors job state records in the state directory. It observes the single task until it reaches a terminal status (`completed`, `timeout`, `cancelled`, `provider-error`, `error`, `stalled`, or `serve-dead`), prints a **compact one-line digest** (`task <job-id>: status=… [phase=…] [failure=kind:…|error=…] waited=Ns`) and exits 0. The digest is deliberately small — it is not a full status dump. For backend, model, session, usage, probes, and result text, use `kusabi-companion status <job-id>` / `kusabi-companion result <job-id>`. If the wait itself stalls or times out waiting for an appearing job within `--appear-timeout`, it exits non-zero (`TaskWaitError` with failure codes: `unknown-job`, `no-job-appeared`, `stalled`, `usage`).

**External Codex notifier boundary (`kusabi-codex-notify`).** For integrations with OpenAI Codex or external orchestrator threads where background tasks run asynchronously, completion detection must not depend on generic kusabi core polling loops. Instead, a thin external Codex notifier boundary (`kusabi-codex-notify`) delivers single-task terminal events at-most-once back to originating Codex threads. Terminal state transitions are observed from finalised job records and dispatched to external threads without introducing external webhook or transport logic into kusabi core.

#### 3.5.7e Kaiba progress retirement at terminal boundaries (kusabi #497)

Kusabi workers write kaiba `progress` rows through kaiba's MCP server. A row is stamped with the worker's identity and, when the backend can inject it, the kusabi job id (`KAIBA_JOB`). Without retirement those rows outlive the job and describe obsolete work until kaiba's TTL prunes them. This section defines the kusabi half of #497 / kaiba#33: stamp the rows, then retire them at the authoritative terminal boundary for the job.

**Exact command.** kaiba PR#34 ships a synchronous one-shot CLI, `kaiba-progress-retire --job <id>` (`pyproject.toml` maps the console entry point to `kaiba.server:progress_retire_main`). It exact-matches `progress.job`, leaves `NULL` and other-job rows untouched, is idempotent, rejects ids outside `^[a-zA-Z0-9_-]+$`, and returns nonzero on a missing, wrong, or unopenable DB. kusabi NEVER opens kaiba's database directly; the only invocation path is `plugins/kusabi/scripts/kaiba-progress-retire.mjs` → `retireJobProgress({ jobId, env, timeoutMs })`, which calls `spawnSync(bin, ["--job", jobId])` with a bounded default timeout (5s). The binary comes from `KAIBA_RETIRE_BIN` (deployment/tests) and defaults to the `kaiba-progress-retire` name resolved on `PATH`. No machine-specific path is hardcoded. Job ids are validated with the same `^[a-zA-Z0-9_-]+$` policy `applyWorkerKaibaIdentity` uses; invalid/empty ids never spawn.

**Lifecycle — the authoritative terminal boundaries.** Retirement is job-scoped and fires only at a terminal boundary, never on a poll:

- **Job save chokepoint.** `saveJob()` (`job-store.mjs`) is the one write path every dispatcher (opencode, claude, agy, cursor) already goes through. A save whose status is terminal — `completed`, `error`, `stalled`, `timeout`, `cancelled`, `provider-error`, `serve-dead` — retires that job synchronously, before `saveJob` returns. A nonterminal save (`running`, including the ~1s incremental stats saves) never does. The retirement decision is made from the INCOMING save status, captured before cancelled-sticky normalization rewrites it: a nonterminal `running` stats save demoted to an already-persisted `cancelled` verdict is not a terminal boundary and is never re-retired; a genuinely incoming terminal save still retires, repeated terminal saves included.
- **Chain finalization.** `finalizeChainControl()` (`chain-control.mjs`) sweeps every unique job id recorded in the chain's `chain.json` — each round record's `implementJobId`, `reviewJobId`, and top-level `reviewFirstJobId`, plus archived replacement-review seats' `reviewJobId` and `reviewFirstJobId` — Set-deduped so a job id repeated across rounds is retired once. The chain id itself is never a retirement target.
- **Stale-pid stop.** The dead-pid branch of `requestChainStop()` finalises the chain to `cancelled` and performs the same sweep, because that is the one terminal path no chain process survives to run.

The sweep is idempotent (kaiba's retire is idempotent, and duplicate terminal saves simply spawn again) and best-effort. `KUSABI_KAIBA_RETIRE=0` disables retirement everywhere — the opt-out for hermetic fixtures and operators who prefer TTL cleanup.

**Fail-soft, and observable only when actionable.** `retireJobProgress` never throws. A missing executable (`ENOENT`) is a silent no-op: an operator without the kaiba CLI installed must not see retirement noise. Invalid/empty job ids and `KUSABI_KAIBA_RETIRE=0` also never spawn. An ACTIONABLE failure — nonzero exit, bounded timeout, or another spawn error (e.g. `EACCES`) — is recorded as a `companion.kaiba.retire` event on the job's `events.ndjson` (`{ type, code }`, `{ type, timedOut }`, or `{ type, error }`) and nothing else: it never changes the job's verdict, the chain's disposition, or `chain.json`, and it never prevents terminalization.
...
**`metrics-ingest` CLI** (`kusabi-companion.mjs`): flags `--transcript-dir <path>` (default `~/.claude/projects`), `--cursor-usage-dir <path>` (default `~/.kusabi/cursor-usage`), `--codex-usage-dir <path>` (default `$CODEX_HOME/sessions` or `~/.codex/sessions`), `--state-root <path>` (default the kusabi state root, `~/.kusabi`), `--db <path>` (default `<state-root>/metrics.db`), `--dry-run`. A dry run opens `:memory:` instead of the real path — the real database file is never even opened, let alone written — and prints the same summary (files scanned / skipped-unchanged, I/O failures, sessions, turns, chains, rounds, findings, parse failures) that a real run would. The Codex usage ingest reads `token_usage_record` JSONL files (see `codex-usage-ingest.mjs`) and measures only `payload.usage`; cumulative `turn_token_usage`/`thread_token_usage` and `event_msg.token_count` are ignored (counted as `legacyTokenCountRecords`). Input is normalised into the existing disjoint buckets (`input_tokens − cached_input_tokens − cache_write_input_tokens` → `input`, cached fields → `cache_read`/`cache_write`); when the cache breakdown is absent, all three are `NULL`. Reasoning tokens are already included in `output_tokens` and not added separately. Request IDs are namespaced `codex:<response_id>` to avoid cross-client collisions; session IDs are verbatim (`payload.session_id` → `payload.thread_id` → `session_meta.id`) to permit orch_session joins. Source-file skip-cache key uses `#codex-usage:v1` suffix, distinct from the cursor-usage `#cu-v2` and Claude transcript keys. The directory is recursively scanned for `*.jsonl` (including root-level files), tolerates a missing directory (warning + zeroed summary), and is idempotent on repeated runs.
...
**Mixed-backend capacity ladders (kusabi #470).** Under kusabi #470, a single phase's chain array (in `models.chain` or `models.phases.<phase>`) may mix routes from different backends (for example, free opencode routes escalating to `agy/gemini-3.8-flash-high` on provider errors). When dispatching a mixed ladder, `dispatchWithFallback` walks the candidate routes across backend boundaries. Cross-backend session guard: session state is backend-specific (`ses_*` for opencode, UUID for claude/agy), so whenever fallback walks from one backend to another, the previous session is dropped and the new backend dispatches fresh without carrying incompatible session IDs. An explicit `--backend <backend>` flag without `--model` continues to enforce that all phases run strictly on that single backend, failing at command start if configured with a mixed chain. Per-route syntax validation still runs at startup before `createChainDir` (e.g. rejecting invalid route spelling or unsupported `:variant` suffixes). The original per-phase backend selection intent of kusabi #192 (e.g. running implement on opencode and review on agy or claude) remains unchanged.
...
**Classification matches observed phrases only** (`classifyDispatchQuotaExhaustion` in `chain-quota.mjs`, kusabi #453): `Individual
