...
#### 3.5.11 claude backend (fresh dispatch) — implemented (kusabi #184)
...
- **No resume.** The `session` option is ignored and every dispatch starts a fresh `claude -p` session; `--resume` / chain-resume session continuation for the claude backend is Job B. The fresh session id is still recorded on the job record.
